...
public class MatchMakingConnectionListener : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        GameNetworkManager.manager.OnPlayerAdded.AddListener(OnPlayerAdded);
        GameNetworkManager.manager.OnPlayerRemoved.AddListener(OnPlayerRemoved);
        ...
        if (portInfo.Count() > 0)
        {
            ...
            GameNetworkManager.manager.Port = portInfo.Single().ServerListeningPort;
        }
        else
        {
    ...
    {
        GameNetworkManager.manager.StartListen();
    ...
    {
        Debug.Log("Player Added  " + playfabId);
    ...
    private void OnShutdown()
    {
        ...
        foreach (var conn in GameNetworkManager.manager.Connections)
        {
    ...
    {
        Debug.Log("Server Quit After MatchMaking Shutdown");
        yield return new WaitForSeconds(5f);
        Application.Quit();
    }
